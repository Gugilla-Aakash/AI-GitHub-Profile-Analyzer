"""
Tests for GeminiProvider (app/clients/llm/gemini_provider.py).
"""
...
class TestGeminiProviderChat(unittest.TestCase):
    ...
    def test_returns_stripped_response_text(self):
        self.mock_client_instance.models.generate_content.return_value = MagicMock(
            text="  Hello, I'm a friendly response!  "
        )
    ...
    def test_system_instruction_includes_context(self):
        ...
        self.assertIn("<developer_profile_context>", system_instruction)
        self.assertIn("Repo: my-cool-project", system_instruction)
    ...
    def test_current_message_appended_last(self):
        ...
        last_part_text = contents[-1].parts[0].text
        self.assertEqual(last_part_text, "latest message")
    ...
    def test_defaults_missing_role_to_user(self):
        ...
        provider = GeminiProvider()
        provider.chat(context="ctx", history=history, message="hi")
